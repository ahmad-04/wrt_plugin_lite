...
/***************************************************************************
 WRTPluginLiteDialog
                                 A QGIS plugin
 A Lite configuration wizard for the 52°North Weather Routing Tool
                             -------------------
        begin                : 2026-03-04
...
class WRTPluginLiteDialog(QtWidgets.QDialog, FORM_CLASS):
    ...
    def _setup_ui(self):
        ...
        # Calendar popup on date pickers
        self.dateTimeDeparture.setCalendarPopup(True)
        self.dateTimeArrival.setCalendarPopup(True)

        # Dropdown style for comboboxes
        combo_style = """
            QComboBox {
            ...
            QComboBox:focus { border: 1px solid #5ba3d9; }
            QComboBox::drop-down {
            ...
            QComboBox QAbstractItemView {
                ...
                border-radius: 4px;
                background-color: #ffffff;
                selection-background-color: #dceef9;
                selection-color: #1e3a5f;
                padding: 4px;
            }
        """
        self.comboAlgorithm.setStyleSheet(combo_style)
        self.comboVessel.setStyleSheet(combo_style)
        self.comboDataMode.setStyleSheet(combo_style)
        ...
        dt_style = """
            ...
            QDateTimeEdit:focus { border: 1px solid #5ba3d9; }
        ...
        """
        self.dateTimeDeparture.setStyleSheet(dt_style)
        self.dateTimeArrival.setStyleSheet(dt_style)
    ...
    def _load_defaults(self):
        ...
        self.comboDataMode.addItems([
            "Automatic (Copernicus download)",
            "Manual (existing files)",
        ])

        self.comboAlgorithm.setCurrentIndex(0)
        self.comboVessel.setCurrentIndex(0)
        self.comboDataMode.setCurrentIndex(0)
    ...
    def _on_data_mode_changed(self, text):
        self.state["data_mode"] = "automatic" if "automatic" in text.lower() else "manual"
        self._update_data_mode_ui()
        self._update_summary()
    ...
    def _update_data_mode_ui(self):
        automatic = "automatic" in self.comboDataMode.currentText().strip().lower()
        ...
        if automatic:
            ...
            self.lineWeatherPath.setPlaceholderText("e.g. weather_data.nc")
            self.lineDepthPath.setPlaceholderText("e.g. depth_data.nc")
            self.lineCoursesPath.setPlaceholderText("e.g. courses.nc")
        else:
    ...
    def _browse_weather_path(self):
        ...
        if automatic:
            path, _ = QFileDialog.getSaveFileName(
                self, "Select Weather Output File",
    ...
    def _browse_depth_path(self):
        ...
        if automatic:
            path, _ = QFileDialog.getSaveFileName(
                self, "Select Depth Output File",
                self.lineDepthPath.text() or "depth_data.nc",
                "NetCDF Files (*.nc);;All Files (*)"
            )
        else:
            path, _ = QFileDialog.getOpenFileName(
                self, "Select Depth Data File",
    ...
    def _build_summary_text(self):
        ...
        lines = [
            ...
            f"  Arrival time: {'Yes' if use_arrival else 'No'}",
            f"  Arrival:      {self.state['arrival_time'] if use_arrival else 'Not exported'}",
            f"  Forecast:     {self.state['forecast_horizon']} h horizon / {self.state['forecast_resolution']} h resolution",
            f"  Algorithm:    {self.state['algorithm']}",
            "",
            "Vessel",
            f"  Preset:       {self.state['boat_preset']}",
            ...
            f"  Weather:      {self._resolve_weather_path()}",
            f"  Depth:        {self._resolve_depth_path()}",
            f"  Courses:      {self._resolve_courses_path()}",
            f"  Output dir:   {self.state['route_output_path'] or 'Not set'}",
        ]
    ...
    def _collect_form_data(self):
        ...
        self.state["data_mode"] = "automatic" if "automatic" in self.comboDataMode.currentText().lower() else "manual"
    ...
    def _build_config(self):
        ...
        default_map = [min(lats), min(lons), max(lats), max(lons)]

        intermediate_waypoints = [[float(p[0]), float(p[1])] for p in waypoints]
